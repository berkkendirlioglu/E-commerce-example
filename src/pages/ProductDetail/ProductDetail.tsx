...
import { ProductDetailType, RatingStars, VariantsType, MostSellerCards, SizeType, Comments, navBarStore } from "../index.ts";
...
const ProductDetail = () => {
  ...
  const [selectedVariant, setSelectedVariant] = useState<VariantsType[]>([ProductDetail.data.variants[0]]);
  const [selectedAroma, setSelectedAroma] = useState<string>();
  const [selectedSize, setSelectedSize] = useState<SizeType>();
  const [productCount, setProductCount] = useState<number>(1);
  const {setBasket} = navBarStore(useShallow((state) => ({
  ...
  useEffect(() => {
    const filteredVariant = ProductDetail.data.variants.filter((variant) => {
      return selectedAroma === variant.aroma &&
              ...
              <div className={`${styles["sizes"]}`}>
                {uniqueSizes.map((portion) => (
                  <button
                    ...
                    key={portion.id}
                    className={`${styles["size-btn"]} ${selectedSize?.gram === portion.size.gram && selectedSize.pieces === portion.size.pieces ? styles["active"]: ""}`}
                  >
                    {portion.size.gram ? (
                      <>
                        <span className={`${styles["gram"]}`}>
                          {portion.size.gram >= 1000 ? `${(portion.size.gram / 1000).toFixed(1)}KG`: `${portion.size.gram}G`}
                        </span>
              ...
              <div className={`${styles["count-cart-container"]}`}>
                <div className={`${styles["product-count"]}`}>
                  <button
                    ...
                    onClick={() => setProductCount(productCount > 1 ? productCount - 1 : productCount)}
                  >
                ...
                <div className={`${styles["cart-button-wrapper"]}`}>
                  ...
                  <button onClick={AddBasketProduct} className={`${styles["cart-button"]}`}>
              ...
              <Accordion title="BESİN İÇERİĞİ">
                ...
                <div className={`${styles["ingredients-content"]}`}>
                  {ProductDetail.data.explanation.nutritional_content.ingredients.map(
                    (nutritionals,index) => (
                      <p key={index} className={`${styles["ingredients-aroma"]}`}>
                        {nutritionals.aroma}:{" "}
                ...
                {ProductDetail.data.explanation.nutritional_content
                  .amino_acid_facts && ProductDetail.data.explanation.nutritional_content.amino_acid_facts.ingredients ? (
                  <div key={"facts"} className={`${styles["facts-table"]}`}>
                    ...
                    {ProductDetail.data.explanation.nutritional_content.amino_acid_facts.ingredients.map(
                      (facts,index) => (
                        <div key={index} className={`${styles["facts-content"]}`}>
                          <span className={`${styles["facts-name"]}`}>
                            {facts.name}
                          </span>
                          <span className={`${styles["facts-value"]}`}>
          ...
          <div className={`${styles["product-comments-wrapper"]}`}>
            ...
            <div className={`${styles["comments-header"]}`}>
                  ...
                  <div className={`${styles["avarage-comments"]}`}>
                    <div className={`${styles["avarage-comments-content"]}`}>
                      <p className={`${styles["avarage"]}`}>{ProductDetail.data.average_star}</p>
                      <RatingStars rating={5}/>
                      <p className={`${styles["comment-count"]}`}>{ProductDetail.data.comment_count} YORUM</p>
                    </div>
                  </div>
            ...
            <div className={`${styles["comments"]}`}>
              ...
              <div className={`${styles["total-comments"]}`}>
                <div className={`${styles["total-comments-count"]}`}>Yorum ({ProductDetail.data.comment_count})</div>
              </div>
